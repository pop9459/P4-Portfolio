...
This file is auto-generated by `combine_portfolios.py`.
 ...
 - updated at: 23-09-2026 17:26:17
...
- Module: TICS Semester 2 · Period 4 - _AI Odyssey: AI2nnovate - Transforming Tomorrow_
...
_What the interviews did to this claim:_ partly supported, and partly redirected. Only P3
...
about her data and yet **not about tracking at all**: what she objects to is _other people
seeing_ what she listens to, on Spotify and on her Kindle. That is social exposure, not
...
_What the interviews did to this claim:_ nothing. Not one of the three raised it
...
_What the interviews did to this claim:_ supported it indirectly and sharply. All three
...
is evaluated in week 4.7: beating _random_ is trivial, and the question worth asking is
...
- Choosing a _content-based_ approach answers (1) and (2) at once — similarity is computed
  ...
  Content-based scoring is in fact _more_ prone to repetition than collaborative filtering,
...
- Doing the interviews late was still worth doing. Two of the three answers pushed back on
  my framing — P1 cared about being _seen_, not about being tracked, and nobody mentioned
...
- _"owns their music files"_ — the input is a local folder, not a streaming catalogue.
- _"only what is already inside those files"_ — artist, title, genre, year, tempo and album
  come from the embedded tags. No lookups, no external metadata service.
- _"nothing ever leaves their computer"_ — no accounts, no telemetry, no cloud, and no
...
_Challenge:_ his player has a shuffle button and nothing else. Shuffle puts a doom-metal
track after a lo-fi study track, so he ends up building playlists by hand and then playing
the same four playlists forever.
_Need:_ something that plays continuously and stays roughly where he started, without him
...
_Challenge:_ she liked the "radio" feature of her old service and misses it, but she is not
willing to hand over a listening profile to get it back. Every offline player she tried
offers shuffle or nothing.
_Need:_ the radio experience without the account behind it, plus reassurance that the
program is not quietly phoning home — something she can check by the fact that it works
with the network off.

### Notes on the week's assignments

- Both profiles converge on the same feature: an endless queue of _related_ tracks, not a
...
honest about being an _AI_ assignment in the CS50 sense: it is a similarity-and-search
...
— it builds a _finite, specified_ playlist, whereas both user profiles asked for an endless
...
- Idea 2 surviving in reduced form is the most useful thing this step produced: a rejected
  idea kept as a _component_ rather than a competitor.
...
- **It kept playing the same artist.** Not a scoring bug — a track sharing genre _and_
...
`track_similarity(a, b)` compares two tracks' metadata _directly_, as four independent 0-1
...
"no genre tag" as _some_ value. Whatever value it picks, every untagged track becomes a
...
and renormalizes the remaining weights over what is left. That is _impossible_ to express
...
zero-year gap. An explicit decay over the absolute _difference_ has no such asymmetry.
...
bass" or "indietronica" that are similar to _nothing_ under a flat scheme. Collapsing them
...
_trailing run_ instead, which a dominant artist walks straight past: with a cap of 3,
...
**Work-key cooldown.** Suppresses other _versions_ of a recently played song. "Thunderstruck",
...
the _lead_ credited artist, so `"Skrillex - Rumble"` and `"Skrillex, Fred again.. - Rumble"`
...
original _seed_ track (`SEED_ANCHOR_WEIGHT = 0.3`). The blend is applied to the two
...
| Queue-assembly filters                | Fix saturation by re-weighting the scoring                     | The scores are correct — a same-artist same-genre track genuinely _is_ the most similar. The problem is what to do with that, which is an assembly question                                                                 |
...
- The distinction between _scoring_ and _assembly_ is the structure of the whole
...
it. They answer _different_ questions, and reaching for the wrong one is why the question
stayed open so long.

**1. `tools/recommender_report.py` — falsification.** Measures the _shape_ of what the
...
_proxy_, and its limitation is specific: albums are strongly artist- and year-coherent, so
it treats genre as largely redundant with artist and cannot credit a term whose job is
bridging _between_ artists.
...
is the only instrument here that judges listening _quality_. It is also slow — a few minutes
...
| `shuffle_genre` — random _within the seed's genre_   | 10-2   | 8/20 | **0.039**   |
...
term works — that is a filter, not a recommendation. Beating _same-genre_ shuffle means the
...
_improves_ album retrieval from 0.766 to 0.813, removing genre changes almost nothing
...
there is _favours keeping_ it. The weights were left at genre 0.45 / bpm 0.25 / artist 0.20
...
to _enforce_ that nothing in the product ever reads the listening-verdict log — the privacy
...
_The 10-minute pitch in written form._
...
10-2 over random shuffle _within the same genre_ — which is what proves it is doing
...
| _Might have:_ local song database                   | Yes — one JSON library, though not the SQLite that was proposed                                                                     |
| _Might have:_ enable/disable flags                  | Yes — `enabled` is the eligibility gate throughout                                                                                  |
| _Might have:_ preprocessing pipeline                | Yes — scan, canonicalize, derive features on load                                                                                   |
| _Might have:_ actual playback                       | Yes, and this is where the project most exceeded its plan                                                                           |
...
I think that during this project I followed the Design Thinking procedure quite well. I did
put a lot of time into just thinking about how the overall structure and the final result
should look. I also took a lot of time to write down the design and validate the reasoning
behind my decisions. I feel like thanks to this I was able to move through the actual
implementation of the project quite quickly.

One thing I still feel is true is that an initial design is never fully reflected in the
...
scenarios where it is frowned upon, I feel like it is not worth it to try to completely
ignore it. Rather than that, I want to try to learn how I can use this tool to work in my
interest.

#### What worked well

I definitely like to keep updated on the latest things experienced developers are saying
about using AI tools to achieve — for lack of a better word — "results". This phenomenon is
still very unstable in the industry and the best "ways" change from month to month.

Currently the best workflow seems to be to use a coding harness like Claude Code
specifically for code generation and system access. Other than that, it is important that
you verify its high-level decisions and debate the choices if they feel off.

Another big component in this workflow seems to be planning, designing and documenting — to
make sure the AI agent has a clear overview of everything that should be done, how it should
be done, and what was done before. The coding harness usually manages a Markdown file which
I like to think of as the memory for the agents between sessions. Alongside that I like to
have human-facing documents where the AI can store my decisions and logs.

The last thing is to embrace as much automation as possible. The whole point of AI is to
speed up development, so it is a big mistake to waste time and brain power on repetitive
tasks like copy-pasting code. The brain power should be purely focused on making the
decisions, and on making sure the AI's decisions are what I actually want.

#### What I would do differently

For this project I kind of streamlined the development, but for future projects I would
integrate more planning tools like Linear to keep better track of the progress, issues,
tasks and ideas. It would also allow for further automation of certain tasks, which could
result in even cleaner organization.

The other major thing would be to move to a local model, as they get more efficient with
their resources and hardware gets more affordable and performant. This would solve many
issues like privacy, price, or the ethics of supporting an unnamed AI company that is
causing global issues for the sake of slightly more profit.

#### Working with AI assistance

I have been quite opposed to and troubled by AI usage in the past, which was part of the
reason for resitting this project. Since then I have figured that this shift in the industry
is not going away and that it would be unwise to ignore it.

I was afraid that it degrades my skills over time as I use it. As a solution to this dilemma
I have decided that I will continue being smart. Looking back, I realized that it is not
only the writing of the code that I like doing — it is the engineering process around it,
which still remains my responsibility. I would therefore hope, for now, that the "it is a
multiplier of your skills" argument is true, and that this can now let me achieve things I
would not have even thought of doing before.
